export default class FishingTileEvents {
  constructor(scene, socket, worldGrid, player) {
    ...
    this.socket.on("occupySuccess", ({ x, y }) => {
      ...
      const tile = this.worldGrid.getFishingTileAt(x, y);
      if (tile) {
        this.player.fishingController.currentFishingTile = tile;
        this.player.stopMoving();
        this.player.fishingController.startFishing(tile);
      }
    });
  ...
  releaseFishingTile() {
    ...
    this.player.fishingController.currentFishingTile = null;
  }
}
